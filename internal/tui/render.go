...
func headerLines(snap *Snapshot, ropts RenderOptions) []string {
	...
	// Inside the bold+inverse strip the glyph carries no SGR of its own —
	// any escape between the re-assert and the ● would break the strip
	// (SGR is not a stack) — so the dot takes the strip foreground and only
	// the label keeps the state color. No leading space: spin already
	// ends with one before the chip.
