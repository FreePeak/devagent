...
// regression guard). RunGuard logs the computed delay ("resuming session …
// in %dms"), so the contract is asserted on that literal — deterministic
// under load, unlike the previous wall-clock bound that failed when the
// suite starved the runner (the #262 CI-load-flake class; any bound high
// enough to survive load can no longer catch the 2000ms default).
...
`)
	...
	if !strings.Contains(errOut, "resuming session ? in 0ms") {
		t.Fatalf("zero base delay not applied verbatim: stderr %q", errOut)
	}
}
