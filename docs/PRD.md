...
| FR-GO-15 | Cutover: production entrypoint flips to Go, release artifacts, self-hosting soak gate | M | [#204](https://github.com/FreePeak/devagent/issues/204) — post-merge repo test gate is configurable via `SELFBUILD_TEST_CMD` (#230, default `npm test`; flip to `go test ./...` at FR-GO-16) |
...
*Last updated: 2026-09-08 (loopdriver #230 — the post-merge repo test gate is configurable: `LoopConfig.TestCmd` / `SELFBUILD_TEST_CMD` (default `npm test`, byte-identical to the bash driver), word-split like the dispatch bins and execed through the ctx-cancellable `runCmd` wrapper as `runRepoTests`; required before FR-GO-16 (#205) deletes the Node suite — the loop then launches with `SELFBUILD_TEST_CMD="go test ./..."` (docs/SELF-BUILD-LOOP.md soak section, §22.4 FR-GO-15 row).)*
