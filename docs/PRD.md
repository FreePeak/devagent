...
| FR-CTRL-03 | Every dispatch through the control API goes through the same pipeline/budget/gate machinery as CLI dispatch — the API is a transport, not a bypass. FR-HAND-03 (#145): `POST /dispatch` accepts `autoPr` (TUI sends true; scripts may pass false) and threads it into the spawned `devagent task --auto-pr` argv only when `GITHUB_TOKEN` is set — without the token the run stays local and the response names the token/PR step as the next action | M |
...
*Last updated: 2026-09-08 (FR-HAND easy handoff #145 — 1 setup + 1 use: README/init cold path is exactly `devagent init` → `devagent tui` (with optional `start` alias); TUI dispatch sheet `n` (FR-HAND-02) + approve sheet `g` via POST /approve (FR-HAND-07) with the paused 'ask' task surfaced on /status; POST /dispatch threads autoPr through dispatchArgv to the spawned `devagent task --auto-pr` argv, gated on GITHUB_TOKEN with a token next-action note when absent (FR-HAND-03); init scans omp/claude/opencode/pi worker chips with install hints (FR-HAND-04), flips herdr.enabled on when the binary is present and the key is unset (FR-HAND-05, FR-VIS-01), and calls ensureOrcaRepo without provisioning worktrees (FR-HAND-06); TUI look/feel bar stays with #146; earlier same day: 2026-09-08 (pane-run --timeout typed as Int in wiredTypedFlags + addFlags no longer drops required marking on typed flags — root cause of the loop-167 indefinite hang inside paneRunDispatch; +wiring regression test; §22 Go migration: G0+G1 COMPLETE — all 16 FR-GO requirement rows shipped to main via parallel agent waves, 26 Go packages green (build/vet/test + golangci-lint v2 + windows cross-compile), issues #190–#203 + #223 closed, PRs #208–#218 + #221–#226; remaining: FR-GO-15 cutover + soak (#204 — gated on the Go loop driver surviving one full live selfbuild iteration per DECISION.md) and FR-GO-16 Node retirement (#205 — deletion gated on soak); §19.7 added: five-scout internet research pass (~90 sources) distilled into a 15-item prioritized adoption backlog — docs/research/2026-09-07-internet-scout-synthesis.md + five scout docs; §22 added: full Go migration of the core — operator decision, FR-GO-01..16, phases G0/G1/G2/G3 tracked via master issue #207 + #190–#206; risks R8–R10 added; §17 roadmap carries the migration note; earlier same day: PRD §18 Q11 resolved — `.devagent/AGENTS.md` auto-load ships with the one-time per-repo trust confirm — `context.agentsMd` config, `loadAgentsMd` loader, `devagent trust agents-md` CLI; task tracker moved from the static §17 Phase 4 backlog to priority-labeled GitHub issues — selfbuild loop is issue-first, prd-curator reconciles the tracker, every PR lands with its PRD state update per the PRD-per-PR policy; see docs/SELF-BUILD-LOOP.md "Tracker + PRD policy")*
