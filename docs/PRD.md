...
---
*Last updated: 2026-09-12 (tracker maintenance: closed stale issues #275, #277, #278, #319 with evidence) — all four open issues were verified stale at HEAD (203231f) before closing: #275/#277 (tui test-double races) no longer reproduce — `go test -race ./internal/tui -count=1` passes clean and both named tests pass under `-race -count=3`; #278 (config Load tests) — `go test ./internal/config/ -count=1` passes with TestLoadDefaults, TestLoadInvalidAPIMaxAttempts, and TestLoadEnvInfinityAPIMaxAttempts green, so `resilience.apiMaxAttempts` validation and the test expectations are consistent with Load's behavior; #319 (competitor scout follow-up) — the migration-moat claim is narrowed in §4.2 and the workflow-mechanics imports are recorded in §19.8, so the issue's two actionable outcomes are absorbed by the PRD. No code change; tracker state only.*
