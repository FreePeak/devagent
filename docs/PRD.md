...
> Both sweep probes are three-valued (2026-09-12): a base branch counts as
> `gone` only on gh's own not-found answer, and the landing listing reports
> "no landing commit" only when it actually answered — transport noise
> (network failure, 5xx, rate limit) is `unknown`, so the sweep acts on
> nothing (`untouched`/`base-unknown`, `untouched`/`landing-unknown`) and the
> merge-queue gate reads it as alive instead of parking a mergeable PR
> (internal/orchestrator/autopr.go).
> No auto-close runs before an age floor (2026-09-12): a definitive verdict
> alone no longer closes a PR — `closeAgeFloorHours` (24h, capped by
> `prHygiene.graceHours` so a long configured window never delays cleanup by
> months) must also have elapsed, and a PR with an unknown age is never
> closed. Fresh PRs are reported `untouched` (`base-superseded-age-floor`,
> `landing-age-floor`) carrying the verdict that will close them once they
> age past the floor.
...
> 2026-09-12 (landing evidence): the ship gate's artifact half. A merge
> verdict that rests on a pull request must show that pull request's touched
> implementation files on main via the `gh api` files/tree reads (docs
> surfaces exempt), or the iteration records the non-productive
> `landed-without-artifact` row with the tracker issue left live; a pick-time
> fingerprint preflight then refuses to re-dispatch that goal — retiring the
> queue claim as `failed` with the preflight detail (the shape gate's
> lease-recycling precedent) — because the same MERGED stamp would otherwise
> re-ship the same nothing (`internal/loopdriver/run.go`, `artifacts.go`).
> Motivation: loop 276
> recorded `ok` for PR #342 (its auto-cleanup-snapshot merge never carried the
> new test file to main) and loop 277 burned a full iteration re-running that
> already-"landed" goal.
>
> 2026-09-12 (fallback-route landing evidence): the evidence subject above
> exists only where a tracker pick produced it. A queue-delivered or PO/LLM
> goal never came through a pick, so a goal that named the pull request to
> land (`Goal: Land PR #344`) recorded `no-pr` even though its worker merged
> that pull request mid-run — loops 270/271/274/280, four false non-productive
> rows in eleven passes against a starvation limit of five. The driver now
> derives the subject from the goal text with the same merge-verb-near-PR
> matcher the pick path parses, under the same pick-time OPEN guard, so the
> derived pull request rides the identical `evidencePR` /
> `landedArtifactsVerified` wiring and a stale or incidental mention still
> self-disqualifies. The `no-pr` early return also retires its queue claim
> (`failed` + the `no-pr` detail, the shape gate's lease-recycling precedent)
> instead of leaving it to re-claim and re-burn the row every lease — a
> tracker-path no-op, where `queued == nil`.
>
> 2026-09-12 (landing certification: the merge stamp is the evidence): the
> artifact gate above recognizes a merge only through a subject the OPEN guard
> admitted *before* dispatch, so an iteration whose pull request merged
> mid-run after a reopen — loop 282: #345 merged 04:39:14Z inside its own
> window while gh still read it CLOSED at dispatch — recorded non-productive
> with nothing to certify. `prState` now decodes `state,mergedAt` from one
> `gh pr view` (`prView`; all five OPEN gates read the state half unchanged),
> and the iteration window is what qualifies the stamp: a pull request the
> goal names whose merge falls inside `[iterStart, now]` certifies the ship
> (`ok`, the artifact gate still included) with no `PR opened:` line and on
> the failed-dispatch path too, while a merge before `iterStart` certifies
> nothing. The same read refuses a dispatch outright when the goal IS a
> `Land PR #N` directive naming a pull request that had already merged before
> the iteration began (loop 280's stale goal re-landing a merged pull
> request): non-productive `skipped` row, queue claim retired, no worker
> burned. The refusal binds to the directive's *position* — the statement
> must open with it — because a goal that merely mentions a pull request is
> not asking the loop to land it: a mid-prose quote (loop 281) must still
> dispatch, and the tracker's implement template embeds the issue TITLE in
> its leading clause, so a title that reads like a merge directive ("… gap in
> PR #345 handling") would otherwise refuse an issue that is re-picked and
> re-refused every iteration — five non-productive rows, the starvation halt,
> and no queue claim to retire. Pinned by
> `TestRunLoopGoalNamedMergedInWindowRecordsShip` (loop 282's window → `ok`),
> `TestRunLoopGoalNamedMergedAfterNoPRRunRecordsShip` (the rc-0 no-pr path),
> `TestRunLoopStaleGoalHeadMergedPRSkipsDispatch` (the directive refusal),
> `TestRunLoopGoalNamedOutOfWindowPRRecordsNoPR` (out-of-window → the
> unchanged `no-pr`),
> `TestRunLoopTrackerTitleReadingLikeMergeDirectiveStillDispatches` (the
> tracker title) and
> `TestRunLoopGoalQuotingMergeDirectiveMidProseStillDispatches` (the
> mid-prose quote). Named ceiling, not fixed: the record-time scan is
> mention-level (any `PR #N` in the goal), so a goal that only quotes a pull
> request which happens to merge inside the window certifies on that merge;
> the window and the artifact gate are the bounds
> (`internal/loopdriver/run.go`, `issues.go`).
>
> 2026-09-12 (the rescue reaches CLOSED-but-unmerged pull requests): the
> driver-side verify-and-merge rescue — and the goal-text derivation feeding
> it — only ever accepted a subject that read `OPEN` *before* the dispatch, so
> the false closes were unreachable: PR #346/#347 (auto-closed by the sweep's
> `BaseBranchGone(main)` transport-noise bug minutes after they opened, green
> and mergeable) and loop 282's #345 (CLOSED at dispatch, the worker's job
> being to reopen it) all recorded `no-pr` with a landable branch left closed.
> `prView` now also decodes `baseRefName`/`headRefName`; a goal-named pull
> request that is CLOSED, unmerged, based on `main`, with its head ref still
> on origin becomes the merge subject, and `verifyAndMergeRescue` reopens it
> (`gh pr reopen`, repo-scoped, 60s-bounded, drain-tolerant), re-reads the
> state — the reopen is verified, never assumed — and only then merges through
> the ordinary `mergePRBounded` path. Every other close lands nothing: a
> superseded/other base, a head ref gone from origin, a merge stamp, and a
> reopen gh refuses all return 0 rather than fabricating a ship. Pinned by
> `TestRunLoopClosedUnmergedGoalPRReopenedAndMerged`,
> `TestRunLoopClosedGoalPRLandsNothingWhenNotReopenable` (base-superseded,
...
| FR-VAL-04 | Chaos soak: nightly fault-injection scenarios — SIGKILL driver mid-iteration (stale-lock break), SIGKILL worker mid-run (attempt 2/3 retry), fake provider hang (watchdog kill), network blackhole during state push (deferred push), repeated failure (circuit breaker) — each asserting recovery + correct ledger classification | S | 🔶 partial (2026-09-12): the deterministic seeded fault-injection simulation of the run cycle landed as `TestSeededFaultInjectionSoak` / `TestSeededTimeoutWallRecovery` / `TestSeededBreakerTrip` in `internal/loopdriver/faultinject_test.go` — a math/rand/v2 PCG stream turns a seed into a fault script (worker-error, dispatch-wall timeout, no-pr) that the fake `devagent` binary serves one task dispatch at a time; the real `RunLoop` consumes it over the golden-soak fixture and the ledger must match a contract-derived prediction (rc≠0 → `failed` + breaker bump, rc-0 without PR evidence → `no-pr` with streak preserved, ship → `ok` + streak reset, breakerAt consecutive failures → exit 1). Remaining scenarios: SIGKILL driver/worker, provider hang, network blackhole ([#292](https://github.com/FreePeak/devagent/issues/292)) |
...
*Last updated: 2026-09-12 (both green heads landed straight onto main — no pull request, no re-implementation) — the zombie sweep was still armed when PR #347 and PR #346 were opened, and its `BaseBranchGone(main)` transport-noise bug auto-closed both within minutes, so the two verified heads were landed straight onto main instead: `1f2ef8c` (`devagent/TASK-mtxyflr1-fq6y` — three-valued hygiene probes plus the auto-close age floor) fast-forwarded main, then `635214d` (`devagent/TASK-mtxwr39q-nva6` — the landing-certification merge window) merged on top. The code paths are disjoint (`internal/orchestrator` vs `internal/loopdriver`) and only `docs/PRD.*` overlapped. Neither PR was reopened or closed by this landing; both stay CLOSED, unmerged as pull requests. Verified on the merged tree: `go test ./...` green, including `TestAutoMergeTreatsBaseProbeNoiseAsAlive` and `TestGhNotFoundOnlyForRealGone`, and `git merge-base --is-ancestor` true for both heads.
*Last updated: 2026-09-12 (zombie-PR hygiene probes are three-valued, with an auto-close age floor) — both gh probes behind the `pr-hygiene` sweep collapsed "gh never answered" into a decisive answer: `BaseBranchGone` returned `err != nil`, so a network hiccup, 5xx, or rate-limit response on the `branches/<base>` lookup read as a dead base and the sweep CLOSED a live PR as `base-superseded` (PR #346: created 05:27:21Z, closed 05:28:37Z, 76s later, never merged); `landingCommitOnMain` returned empty on a failed or unparseable commit listing, so transport noise read as "no landing commit" and the PR was closed as `superseded` with an explicit "not shipped" comment. `ProbeBaseBranch` (internal/orchestrator/autopr.go) now answers `BaseAlive` / `BaseGone` / `BaseUnknown`, judging "gone" only on gh's own not-found stderr (`ghNotFound` reads the diagnostic, never the argv-embedding message, whose ref names may themselves contain "404"; `GhError.Code` is the gh exit code and is never consulted), and `landingCommitOnMain` returns `(sha, ok)` with `ok` false whenever the listing failed. On `unknown` the sweep records `untouched`/`base-unknown` or `untouched`/`landing-unknown` and acts on nothing; the merge-queue gate treats `BaseUnknown` as alive, so a probe hiccup no longer parks a mergeable PR. Both auto-close arms now also wait out an age floor: `closeAgeFloorHours` (24h) capped by `prHygiene.graceHours`, with an unknown PR age treated as floored — a fresh PR is reported `untouched`/`base-superseded-age-floor` or `untouched`/`landing-age-floor` and the `--grace-hours` help states the floor. Regression-pinned by `TestSweepTaskPrHygiene` (base-probe noise, fresh and unknown-age floors), `TestSweepTaskPrHygieneLandingEvidence` (landing noise, unparseable listing, landing floor), `TestAutoMergeTreatsBaseProbeNoiseAsAlive`, and `TestGhNotFoundOnlyForRealGone` — each fails against the pre-fix behavior.*
*Last updated: 2026-09-12 (landing certification: the window, not the pre-dispatch stamp, is what certifies a ship) — the ship gate could only certify a pull request the OPEN guard had admitted before dispatch, so loop 282's reopen-and-merge (#345 merged 04:39:14Z inside its window) recorded non-productive: `gh pr view --json state` never said *when* a merge happened. `prView` now reads `state,mergedAt` in that one call (all five OPEN gates unchanged, `prState` a thin wrapper), a goal-named pull request whose merge stamp falls inside `[iterStart, now]` certifies `ok` (artifact gate included) on both the failed-dispatch and the rc-0 no-pr paths, an out-of-window stamp certifies nothing (loop 281), and a goal that IS a `Land PR #N` directive whose target had already merged before the iteration is refused pre-dispatch — `skipped` row, claim retired, no worker burned (loop 280). The refusal binds to the directive's position (the statement must open with it), so a tracker implement goal carrying an issue title that reads like a merge directive still dispatches instead of halting the loop through re-pick/refuse cycles. Pinned by `TestRunLoopGoalNamedMergedInWindowRecordsShip`, `TestRunLoopGoalNamedMergedAfterNoPRRunRecordsShip`, `TestRunLoopStaleGoalHeadMergedPRSkipsDispatch`, `TestRunLoopGoalNamedOutOfWindowPRRecordsNoPR`, `TestRunLoopTrackerTitleReadingLikeMergeDirectiveStillDispatches` and `TestRunLoopGoalQuotingMergeDirectiveMidProseStillDispatches`; ceiling — the record-time scan is mention-level, so the window and the artifact gate are what qualify a mention.*
*Last updated: 2026-09-12 (fallback-route landing evidence: the ship verdict off the tracker path) — `pick.mergePR` is only ever set by a tracker pick, so a queue- or PO-delivered goal (`Goal: Land PR #N`) had no evidence subject at all: loops 270/271/274/280 each logged `falling back to LLM selection`, each dispatched a goal naming the pull request to land, each of those pull requests merged inside its own iteration window, and each recorded the non-productive `no-pr` row — four false negatives in eleven passes against a starvation limit of five, with the Q27 already-shipped guard equally blind. `runIteration` now derives the merge target from the goal text (`goalMergePR`, the `mergePickRe` researchPick already parses) before the task dispatch, under the pick-time `prState == OPEN` guard, so the derived pull request rides the existing `prMerged` → `evidencePR` → `landedArtifactsVerified` wiring and records `ok` with its artifacts verified — never `no-pr`, while a stale mention self-disqualifies and a derived PR still open at record time is the driver-side verify-and-merge rescue's subject. The `no-pr` early return now also retires its queue claim `failed` with the `no-pr` detail (the shape gate's lease-recycling precedent) instead of leaving it to re-claim and re-burn the row every lease; tracker iterations are untouched (`queued == nil`). Pinned by `TestRunLoopGoalNamedOpenPRMergedRecordsShip`, `TestRunLoopGoalNamedMergedPRRecordsNoPR` and `TestGoalMergePR`; `internal/loopdriver/DECISION.md` records the decision.*
*Last updated: 2026-09-12 (landing evidence: a ship needs artifacts on main) — `merged = shipped` trusted gh's status stamp alone, and three consecutive iterations (276–278) proved the stamp lies: loop 276 recorded `ok` for PR #342 (merged only as an auto-cleanup snapshot — the test file it was meant to add never reached main) and loop 277 burned a full iteration re-running that already-"landed" goal. The driver's verify-and-merge path now verifies the artifact half: `landedArtifactsVerified` (internal/loopdriver/artifacts.go) lists the merged PR's touched files and main's tree over the `gh api` files API and requires every touched implementation file to exist on main (docs surfaces exempt — a PRD regen ships alone; a file the PR `deleted` is no claim, and a truncated tree list reads as cannot-say); a mismatch records the non-productive `landed-without-artifact` row and returns before the `ok` write, so the queue claim and tracker issue stay live, and a pick-time preflight greps the ledger for the goal's fingerprint (the Q27 guard's 60-char key) on those rows and skips the dispatch instead of re-reading the same MERGED stamp — retiring that claim as `failed` with the preflight detail (the shape gate's lease-recycling precedent) while the issue stays open. A gh that cannot answer logs and passes — evidence gate, not a second availability gate — and both decoders were checked against the real repository before the fixtures were trusted. Covered by `TestRunLoopMergedWithoutArtifactRecordsEvidenceRow`, `TestRunLoopMergePickRescuedByDriverSideMerge` (artifact-backed ship), `TestRunLoopPreflightSkipsGoalLandedWithoutArtifact`, `TestLandedArtifactsVerifiedIgnoresRemovedAndTruncated` and `TestLedgerLandingFingerprint`; the goal-shape fix PR #343 was recovered in the same iteration (re-opened, head 50891d5 verified with `go test ./...` in a throwaway worktree, merged as 38e6513).*
*Last updated: 2026-09-12 (goal-shape follow-up: queue-claim retirement + producer scope) — the shape gate's word cap binds every producer, not just PO output: a human goal from `POST /dispatch` or a scout-enqueued queue row past the 120-word statement budget is refused at the boundary too (the PO prompt is the one statement contract all dispatched goals trace back to; DECISION.md divergence #5 records the reasoning). The same pass closed the leak the cap opened: a queued claim whose goal is off-contract is now retired as `failed` with the rejection detail as its LastError at BOTH invalid sites (the goal-file gate and the taskDispatch boundary) — pre-fix the claim sat until its 2h lease lapsed, re-claimed, and re-burned an invalid row every cycle, walking the loop into the starvation halt with the row still live. Covered by `TestRunLoopInvalidQueuedGoalRetiresTheClaim` (run_test.go).*
*Last updated: 2026-09-12 (goal shape validated at the dispatch boundary) — `internal/loopdriver`'s `^Goal:` gate was a multiline regex scan: any goal file that merely contained a "Goal:" line — a crashed or garbled research/PO capture with the marker buried mid-blob — passed and burned a full task dispatch on garbage. The goal contract is now a single predicate, `validateGoalShape` (dispatch.go): the text must START with the "Goal: " prefix every producer emits (queue normalization, issue/merge templates, the PO prompt) and carry a non-empty statement of at most 120 words (the PO prompt's statement contract; the driver-appended phase-1 pick rationale rides after the statement and is not part of the word budget). It is enforced twice: at the `taskDispatch` boundary itself — an off-contract goal is refused with the `taskGoalInvalidRC` sentinel (-1, uncollidable with real child exit codes) and the child never spawns — and at the runIteration goal-file gate. `runTaskPhase` maps the sentinel to the established invalid-goal semantics (invalid row, fall-through, breaker never consulted — DECISION.md divergence #5), so a malformed goal can never burn breaker credit. Covered by `TestTaskDispatchValidatesGoalShape` (shape table + spawn-count proof), `TestValidateGoalShape` (predicate boundaries), and `TestRunLoopRejectsMalformedGoalShape` (invalid row + no dispatch) — dispatch_test.go / run_test.go.*
*Last updated: 2026-09-12 (FR-VAL-04 partial: deterministic seeded fault-injection simulation of the run cycle) — the chaos-soak requirement gains its seeded simulation half: `internal/loopdriver/faultinject_test.go` drives the REAL `RunLoop` over the golden-soak fixture with a math/rand/v2 PCG-seeded fault script (`worker-error`, dispatch-wall `timeout`, `no-pr`) served by the fake `devagent` binary one task dispatch at a time; the ledger must match a contract-derived prediction (rc≠0 → `failed` + breaker bump, rc-0 without PR evidence → `no-pr` with the streak preserved, ship → `ok` + reset, `MaxConsecutiveFailures` consecutive failures → exit 1 at exactly that many rows), and every ship must close its issue while every fault leaves it open. The fakes gained `DEVAGENT_FAULT_SCRIPT`/`DEVAGENT_FAULT_STATE` (task dispatch) and `GH_ROTATE_MOD` (pick rotation) — both env-guarded, defaulting to prior behavior. Same seed → same script → same ledger; seeds 1/2/3/7/42 × 8 iterations green locally with the full loopdriver suite. Remaining FR-VAL-04 scenarios (SIGKILL driver/worker, provider hang, network blackhole) stay open on #292.*
*Last updated: 2026-09-12 (live e2e verification of the driver-side verify-and-merge rescue, df4e760) — the merged loopdriver rescue path was exercised live against the real tracker: scratch issue #340 (labeled `selfbuild`) held an open, green pull request; the driver iteration's task dispatch was made to end without shipping evidence (nonzero rc, the loops-270-272 failure class); the driver landed the open PR itself through `mergePRBounded`, recorded the productive `ok` ledger row, closed the issue with the merge evidence, and ran the pr-hygiene sweep in apply mode. This PR is that fixture — its own merge is the rescue's output.*
*Last updated: 2026-09-12 (driver-side verify-and-merge completion, loop-273 pick) — the selfbuild loop stopped one step short of shipping three iterations in a row (270-272): a run finished green with a mergeable PR for its picked issue, yet the ledger read `failed`/`no-pr` and nothing merged the PR. `runIteration` now closes the loop itself: when the task dispatch ends without shipping evidence (nonzero rc, or rc 0 without a `PR opened:` line) and the picked issue still has an `OPEN` pull request — the research pick, a cross-referenced one, or one the run itself opened (`openPROfIssue`) — the driver lands it through `mergePRBounded` (bounded retries of `AutoReviewAndMergeOne`, the #301 CI-Fixer pipeline, with the merged-state re-read as the tiebreaker), records the productive `ok` row, closes the issue, and applies the #336 pr-hygiene sweep in apply mode so zombie duplicates triage against the fresh landing evidence. `LoopConfig.GhRun` injects the gh runner so the orchestrator calls resolve the tracker repo (`GHRepo`) the same way the driver's own calls do instead of gh's cwd inference.
...
(`prMerged`, decoded `gh pr view --json state,mergedAt` — `prView`, with `prState` the thin state reader; the file's own `pickIssue`
