...
# matched the shipped productive set but had no degraded exemption, so a
# provider outage (provider-degraded / operator-* rows) counted toward
...
while :; do
  ...
  LOG="$STATE/logs/loop-$N.log"
  {
    ...
    # Exit 0 — an intentional stop. Exit 1 + hub restart=on-failure
    # resurrects the halt every backoff interval (2026-09-06: 58 hollow
    # loop-106 starts).
    if starved; then
...
Generated by scripts/orchestrator-loop.sh" 2>&1 | tail -1) || {
  ...
  tail -10 "$LOG"
  fails=0
  # Intentional stop -> exit 0 (hub restart=on-failure must not resurrect it).
  [ "${SELFBUILD_MAX_ITERATIONS:-0}" -gt 0 ] && [ "$N" -ge "${SELFBUILD_MAX_ITERATIONS:-0}" ] && { echo "max iters reached"; exit 0; }
done
