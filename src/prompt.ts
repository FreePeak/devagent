 ...
 * by measured held-out impact (Q39 + held-out tier), then drop the
 * lowest-ranked entries whole until the surviving block fits
 * `LESSONS_MAX_CHARS` (config `lessonsMaxChars`). Ranking uses the per-lesson
 * score from the orchestration ledger (`loadLessonScores`, held-out: each
 * lesson is scored only on loops strictly after its first-seen loop, so it
 * cannot lift its own rank on the loop it rode): lines whose excerpt hash has
 * a score sort by score (descending, oldest-first tiebreak) ahead of
 * unscored lines, which keep the existing newest-first order so the recency
 * behavior is preserved when no impact data exists. Never splits a line and
 * never strips content from a kept line, so an optional `predictedImpact:`
 * suffix appended to a lesson round-trips verbatim through the injected
 * digest.
 */
